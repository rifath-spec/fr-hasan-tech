...
import { claimOfferOnWhatsApp, shareToWhatsApp } from '../../utils/share';
...
export const HomePage: React.FC = () => {
  const { navigate, settings, services, offers, isLoadingData } = useApp();
  const [isShareModalOpen, setIsShareModalOpen] = useState(false);

  const activeOffers = useMemo(() => {
    return (offers || [])
      .filter(o => o.active !== false && o.isPublished !== false)
      .sort((a, b) => {
  ...
  return (
    <div className="w-full bg-[#F8FAFC]">
      ...
      {/* 2.2 Featured Special Offers & Deals Showcase (if active offers exist) */}
      {activeOffers.length > 0 && (
        <section className="py-8 sm:py-10 bg-gradient-to-b from-amber-50/40 via-blue-50/20 to-transparent border-b border-slate-200/60">
          <div className="w-full max-w-[1760px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
              <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-100 border border-amber-200 text-amber-900 text-xs font-bold mb-2">
                ...
                <p className="text-xs sm:text-sm text-slate-500 mt-1">
                  Grab exclusive combo deals and discounted tech services. Direct claim via WhatsApp.
                </p>
              </div>
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#062B5C] hover:bg-[#083b7e] text-white font-bold text-xs sm:text-sm shadow-soft-sm transition-all cursor-pointer self-start sm:self-auto shrink-0"
              >
                <span>View All {activeOffers.length} Offers</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {activeOffers.slice(0, 3).map((offer) => {
                ...
                return (
                  <div
                    key={offer.id}
                    className="bg-white rounded-2xl border border-slate-200 shadow-soft-sm hover:shadow-soft-md transition-all flex flex-col overflow-hidden group"
                  >
                    <div className="relative h-44 bg-[#062B5C] overflow-hidden">
                      {displayImage ? (
                        <img
                          src={displayImage}
                          alt={offer.title}
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center p-4 text-center">
                          <Tag className="w-8 h-8 text-amber-300 mb-1" />
                          <span className="text-xs text-white font-bold">{offer.category}</span>
                        </div>
                      )}
                      ...
                      {offer.validUntil && (
                        <div className="absolute bottom-2 left-2.5 bg-slate-950/70 backdrop-blur-xs text-slate-200 text-[10px] font-semibold px-2 py-0.5 rounded">
                          {offer.validUntil}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                      <div>
                        <span className="text-[10px] font-bold uppercase tracking-wider text-[#1E5AA8] bg-blue-50 px-2 py-0.5 rounded">
                          {offer.category || 'General'}
                        </span>
                        <h3 
                          onClick={() => navigate(`/offers?id=${offer.id}`)}
                          className="font-bold text-slate-900 text-base leading-snug mt-1.5 line-clamp-2 hover:text-[#1E5AA8] cursor-pointer"
                        >
                          {offer.title}
                        </h3>
                      ...
                      <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
                        <div>
                          {offer.offerPrice !== undefined ? (
                            <div className="flex items-baseline gap-1.5">
                              <span className="text-base font-black text-[#062B5C]">
                            ...
                            <span className="text-xs font-bold text-emerald-600">Special Package</span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 bg-[#16B95A] hover:bg-[#12A94F] text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer"
                          >
              ...
              {publishedServices.map((service, sIndex) => {
                ...
                return (
                  <div
                    ...
                    className="group bento-card border border-slate-200/90 shadow-soft-sm hover:shadow-soft-md flex flex-col justify-between overflow-hidden bg-white rounded-2xl cursor-pointer"
                  >
